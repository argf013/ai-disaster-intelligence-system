...
export const ImageAnalysisView = ({ onUseInAssessment, currentLocation, onNavigateToShelters }) => {
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch((err) => console.error('Failed to load nearest shelter in ImageAnalysis', err));
  }, [currentLocation]);
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="text-[11px] text-slate-300">
                        ...
                        Available: <b className="text-emerald-400">{nearestShelter.available_capacity}</b> / {nearestShelter.shelter.capacity} beds
                      </p>
                    </div>
                    {onNavigateToShelters && (
                      <button
                        ...
                        className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded text-xs font-semibold flex items-center gap-1.5 transition shadow shrink-0"
                      >
